...
public class NoticeDao {
	...
	public int noticeDelete(String no) {
		int result = 0;
		String query = "delete from home_김진욱_notice\r\n" + 
				"where no = '"+no+"'";
		try {
	...
	public int noticeUpdate(NoticeDto dto) {
		int result = 0;
		String query = "update home_김진욱_notice\r\n" + 
	...
	public NoticeDto getNextNotice(String no) {
		...
		String query = "select a.no, b.title\r\n" + 
				"from\r\n" + 
				"(select min(no) as no\r\n" + 
				"from home_김진욱_notice\r\n" + 
				"where no > '"+no+"') a, home_김진욱_notice b\r\n" + 
				"where a.no = b.no";
		try {
			con = DBConnection.getConnection();
			ps = con.prepareStatement(query);
	...
	public void setHitCount(String no){
		String qurey = "update home_김진욱_notice\r\n" + 
				" set hit = hit + 1\r\n" + 
				" where no = '"+no+"'";
		try {
			con = DBConnection.getConnection();
			...
			System.out.println("setHitCount() 오류 : "+qurey);
			e.printStackTrace();
		}finally {
			DBConnection.closeDB(con, ps, rs);
		}
	...
	public NoticeDto getNoticeView(String no) {
		...
		String query = "select n.no, n.title, n.content, n.attach, m.name, to_char(n.reg_date, 'yyyy-MM-dd hh:mi:ss') as reg_date, n.hit\r\n" + 
				"from home_김진욱_notice n, home_김진욱_member m\r\n" + 
				"where n.reg_id = m.id\r\n" + 
				"and n.no='"+no+"'";
		try {
			con = DBConnection.getConnection();
	...
	public int noticeSave(NoticeDto dto) {
		int result = 0;
/*		
		String query = "insert into home_김진욱_notice\r\n" + 
...
*/				
		String query = "insert into home_김진욱_notice\r\n" + 
				"(no, title, content, attach, reg_id, reg_date)\r\n" + 
				"values\r\n" + 
				"('"+dto.getNo()+"', '"+dto.getTitle()+"', '"+dto.getContent()+"', '"+dto.getAttach()+"', '"+dto.getReg_id()+"', to_date('"+dto.getReg_date()+"','yyyy-MM-dd hh:mi:ss'))";
		try {
			con = DBConnection.getConnection();
			ps = con.prepareStatement(query);
			result = ps.executeUpdate();
